...
class Sorting:
    ...
    def radix_sort(self, array, comp, begin, end):
        # the largest number (k) in array has to be less than array.size^c (n^c) where c is constant
        # if c can be whatever we want there exist such a c that k <= n^c
        # now the complexity of radix sort if O(n*log(n, base))
        # so now we want b to be n to minimize O the it would be O(n)
        maximum = max(array)
        number_of_bytes = len("{0:b}".format(maximum))
        logarithm = int(floor(log2(len(array))))
        if number_of_bytes >= logarithm:
            base = logarithm
        else:
            base = number_of_bytes
        exp = 1
        while maximum / exp > 1:
            self.counting_sort(array, comp, begin, end, exp, base)
            exp *= base

    def counting_sort(self, array, comp, begin, end, exp, base=10):
        n = end - begin + 1
        result = [0] * n
        count = [0] * base
        for i in range(n):
        ...
        for i in range(n-1, -1, -1):
            tmp = array[i] // exp % base
            result[count[tmp] - 1] = array[i]
            count[tmp] -= 1

        for i in range(0, n):
            self.__assign(array, i, result[i])
